...
> **Yinix Is Not unIX** — 向 GNU 递归命名致敬。

一个用 **C++17** 实现的，**基于Linux平台的**功能模拟操作系统，提供交互式命令行 Shell，涵盖进程管理、内存管理、文件系统和设备管理四大核心模块。
...
| 方向 | 说明 |
|---|---|
| 多种调度算法 | 在 RR 基础上增加 FCFS、SJF、优先级调度、多级反馈队列（MLFQ） |
| 抢占式调度 | 高优先级进程就绪时可抢占当前运行进程的 CPU |
| 进程族关系 | 模拟 `fork`/`exec`，支持父子进程树、僵尸进程与孤儿进程回收 |
| 死锁检测与恢复 | 实现资源分配图，检测循环等待；或用银行家算法做死锁预防 |
| 管程（Monitor） | 在信号量基础上封装更高级的同步原语，内置条件变量 |

### 内存管理

| 方向 | 说明 |
|---|---|
| 分页内存管理 | 引入页表，将逻辑地址映射到物理帧，支持 `pageTable` 命令查看 |
| 虚拟内存 | 实现按需调页（demand paging），模拟内存不足时的换入换出 |
| 缺页中断 | 进程访问未装入物理内存的页时触发缺页，调页后恢复执行 |
| 页面置换算法 | 实现 FIFO、LRU、时钟（Clock）等置换策略并可对比效果 |
| 内存分段 | 引入段表，支持代码段/数据段/堆栈段的独立管理 |

### 文件系统

| 方向 | 说明 |
|---|---|
| inode 结构 | 将文件元数据（大小、权限、时间）与文件名解耦，贴近真实 ext 系列文件系统 |
| 磁盘块模拟 | 用固定大小块数组模拟磁盘，文件内容分块存储，支持碎片整理 |
| 文件权限 | 引入 rwx 权限位和用户/组概念，`chmod`/`chown` 命令 |
| 绝对路径解析 | 支持完整多级绝对路径（如 `/home/user/docs/file.txt`） |
| 软/硬链接 | 实现 `ln` 命令，模拟符号链接与硬链接的引用计数 |

### 设备管理

| 方向 | 说明 |
|---|---|
| 设备等待队列 | 设备忙时进程进入该设备的等待队列阻塞，释放后自动唤醒 |
| 中断模拟 | 设备操作"完成"后触发中断，通知进程继续执行 |
| DMA 模拟 | 模拟 DMA 传输过程，设备独立完成数据搬移后再通知 CPU |
| 动态设备注册 | 支持运行时挂载/卸载虚拟设备 |

### 其他

| 方向 | 说明 |
|---|---|
| 多核 CPU 模拟 | 扩展为多个 `runningPid` 槽位，模拟 SMP 并行调度 |
| 进程间通信（IPC） | 实现管道（pipe）、消息队列、共享内存三种经典 IPC 机制 |
| 用户/内核态分离 | 区分特权级，系统调用需陷入"内核态"才能操作资源 |
| Shell 脚本支持 | 支持从文件读取并批量执行命令 |
